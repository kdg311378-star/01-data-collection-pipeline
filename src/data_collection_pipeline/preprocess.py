...
def parse_batch_directory_name(batch_dir: Path) -> datetime:
    ...
    Returns:
        배치 폴더명에서 추출한 날짜와 시각 (UTC Aware Datetime)
    ...
    if BATCH_DIR_PATTERN_RE.fullmatch(batch_dir.name) is None:
        raise ValueError(
            f"interim 배치 폴더명 형식이 올바르지 않습니다. {batch_dir.name}"
        )
...
def validate_input_books(books_df: pd.DataFrame) -> None:
    ...
    if missing_columns:
        raise ValueError(
            f"입력 데이터의 필수 컬럼이 누락되었습니다. {sorted(missing_columns)}"
        )
...
def clean_string_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in STRING_COLUMNS:
        clean_df[column] = (
            clean_df[column].astype("string").str.strip().replace("", pd.NA)
        )
...
def preprocess_books(
    ...
    processed_df["is_available"] = (
        processed_df["availability_text"].map(parse_availability).astype("boolean")
    )
